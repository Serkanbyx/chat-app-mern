 ...
 *
 * The "New chat" CTA is a placeholder until Step 26 wires up the
 * `NewChatModal`. We render it as a disabled control with a tooltip
 * rather than hiding it, so the layout proportions and tab order
 * match what the finished UI will look like.
 */
const EmptyChatPage = () => (
  <div className="flex h-full w-full items-center justify-center bg-gray-50 px-6 text-center dark:bg-gray-950">
    <div className="max-w-sm">
      ...
      <button
        type="button"
        disabled
        title="New chat modal arrives in Step 26"
        className="mt-5 inline-flex items-center gap-1.5 rounded-md bg-brand-600 px-3.5 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-brand-500 dark:hover:bg-brand-400"
      >
